...
import {Spinner,Container,Card,CardTitle,CardText,Row,Col,Button,} from 'reactstrap';
import '../App.css';
...
class LocalData extends Component {
  ...
  componentDidMount() {
    ...
    axios.get('http://newsapi.org/v2/everything?' + 'q=covid19&' + 'from=2020-05-05&' + 'sortBy=popularity&' + 'apiKey=240eb95a24ea43e886bb5f1fb00ffe5c')
      .then((res) => {
        this.setState({
    ...
    axios.get('https://ipapi.co/country').then((res) => {
      ...
      axios.get(`https://corona.lmao.ninja/v2/countries/${res.data}`)
        .then((res) => {
          this.setState({
        ...
        <div>
          <Container fluid>
            <Row style={{ marginBottom: '35px', marginLeft: '5px' }}>
              <Col sm={4}>
                <Button disabled style={{backgroundColor: 'black',color: 'white',width: '100%',}}>
                  <h4>Top News</h4>
                </Button>
                <Card className='shadow-sm' style={{height: '435px',overflowY: 'auto',}}>
                  <TopNewsProvider loading={this.state.loading} articles={this.state.articles}/>
                </Card>
              </Col>
              <Col sm={8} style={{ textAlign: 'center', marginTop: '40px' }}>
                ...
                <Row style={{ textAlign: 'center' }}style={{ marginTop: '45px', marginRight: '10px' }}>
                  <Col>
                    <Card outlinestyle={{borderColor: '#3185F0',borderWidth: '2px',height: '100px',}}className='shadow-sm'>
                ...
                <Row style={{marginTop: '20px', marginRight: '10px',}}>
                  <Col>
                    <Card body inverse style={{backgroundColor: '#333',borderColor: '#3185F0',borderWidth: '2px',height: '100px',}} className='shadow-sm'>
